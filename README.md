# GDGoC IPUT "Gen AI Media Workshop" 準備プロジェクト

本リポジトリは「Gen AI Media Workshop」に向けた、ハンズオン資料および各種成果物を管理しています。

## プロジェクトの目的

マルチモーダル AI（Gemini / Nano Banana 2 / Veo 3）を使用し、大学1・2年生を対象に「プログラムから AI を操作する体験」を提供するワークショップの準備を行います。

## ハンズオン資料 (Google Colab)

- [**`handson_notebook.ipynb`**](handson_notebook.ipynb): 学生が実際に手を動かして学ぶための、Google Colab 用 Jupyter Notebook です。API の基礎（cURLによる取得）から、`google-genai` SDK を使った画像生成（`Modality.IMAGE`）とファイル保存までを含みます。
